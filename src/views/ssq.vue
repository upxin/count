<template>
  ...
  <div class="button-group">
    <el-button @click="$router.push('/dlt')" type="success">去dlt</el-button>
...
// 保存数据
function saveData() {
  localStorage.setItem('ssqSel', JSON.stringify(Array.from(selectedNumbers.value)))
  localStorage.setItem('ssqTable', JSON.stringify(tableData.value))
}
...
const clear = () => {
  ...
  localStorage.removeItem('ssqSel')
  localStorage.removeItem('ssqTable')
}

// 加载数据
const loadData = () => {
  const sD = localStorage.getItem('ssqSel') as string
  const tD = localStorage.getItem('ssqTable') as string
